...
//    @PostMapping("/login")
    public String loginV1(@Valid @ModelAttribute LoginForm form, BindingResult bindingResult, HttpServletResponse response) {
        ...
        if (loginMember == null) {
            bindingResult.reject("loginFail", "아이디 또는 비밀번호가 맞지 않습니다."); // 글로벌 에러 처리
            return "login/loginForm";
        }

        // 로그인 성공 처리

        // 쿠키에 시간 정보를 주지 않으면 세션 쿠키가 된다(브라우저 종료시 모두 종료됨)
...
//    @PostMapping("/login")
    public String loginV3(@Valid @ModelAttribute LoginForm form, BindingResult bindingResult, HttpServletRequest request) {
        ...
        // 세션이 존재할 경우 해당 세션 반환하고 없는경우 신규 세션 생성
        HttpSession session = request.getSession(true); // getSession(true): 기존 세션 반환(없는 경우 생성해서 반환), getSession(false): 기존 세션 반환하거나 없는 경우 null 반환
        // 세션에 로그인 회원 정보 보관
...
//    @PostMapping("/logout")
    ...
    private static void expiredCookie(HttpServletResponse response, String cookieName) {
        ...
        cookie.setMaxAge(0); // 쿠키 지속시간이 0 이기 때문에 해당 쿠키는 만료된다
